...
let search = () => {
  ...
  if ($("#searchingFor").val() == "") {
    ...
    $("#found").append(
      $(
        "<p>Database is empty. Please <strong>add book</strong> or <strong>talk to the databse admin</strong> if you think there is a problem.</p>"
      )
    );
    $("#found").show();
    return;
  ...
  if (found) {
    // $("#found").empty();
    let removeButton = $(
      "<button onclick='removeBook()' id='removeBook'>Remove this book</button>"
    );
    ...
    $("#found").append(
      $(
        "<p>Couldn't find this book. If you think it is there, please contact with the database admin.</p>"
      )
    );
    $("#found").show();
    return;
...
let removeBook = () => {
  ...
  if (couldBeRemoved.length > 1) {
    message =
      couldBeRemoved.length + " books will be removed permanently. Continue?";
  } else {
